...
The net work done on a particle is equal to its change in kinetic energy, but at high speeds $E=\frac{1}{2}m{v}^2$ is not entirely correct. When a steady
net force is applied to an object, the object increases in speed and since it is moving through a distance, work is done and its energy increases. Assuming
the object moves very close to $c$, its energy is equal to $E = Fc$. The momentum is equal to $F = mc$. Combining these equations gives $E=m{c}^2$.
The Kinetic energy of a high speed particle is equal to $E=m_{0}{c}^2-m{c}^2$ where $m{c}^2$ is rest energy. When the energy of a system changes by $\Delta{E}$, the mass changes by

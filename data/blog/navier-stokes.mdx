...
$$
\sum F = \sum F_{surfaces} + \sum F_{body} = \nabla \cdot T + \rho \vec{b}
$$

where $\vec{b}$ is the body forces per unit mass. The momentum expression is now:

$$
\nabla \cdot T + \rho \vec{b} = \frac{d}{dt}\int_{cv}^{}\vec{V}\rho d\boldsymbol{\mathbf{V}}+\int_{cs}^{}\rho\vec{V}\vec{V}\cdot \vec{n} dA
$$
